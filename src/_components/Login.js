      ...
      <Container>
        ...
        >
          ...
          <form onSubmit={ async e => {
            e.preventDefault();
            if (Object.keys(state.creds).map(key => state.creds[key]).filter(value => !value).length > 0) return this.handleError('Not all fields are valid. Please check the inputs above and try again.')
